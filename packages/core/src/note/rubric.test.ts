...
describe("judgeDraft — 원문과 관점의 균형 (ADR-0009)", () => {
  ...
  it("원문 정리가 관점의 두 배를 넘으면 되묻는다", () => {
    const verdict = judge({
      sourceOutline: [
        "원문은 인자가 셋을 넘는 함수를 길게 예로 들며 호출부가 어떻게 읽히는지 한 줄씩 짚어 나가고 그때마다 생기는 불편을 차례로 열거한다.",
        "같은 인자 묶음이 여러 함수로 번지는 과정을 단계별로 보이고 그때마다 바뀌는 시그니처를 화면에 나란히 나열해 하나씩 견주어 보인다.",
        "객체로 묶은 뒤의 호출부를 앞의 것과 나란히 놓고 무엇이 짧아졌는지 항목마다 하나하나 견주어 보이며 줄마다 설명을 덧붙여 나간다.",
        "이름 붙은 속성이 생기면서 호출부에서 인자 순서를 기억할 일이 사라진다는 점을 사례를 바꿔 가며 여러 번 되풀이해 강조하고 넘어간다.",
        "타입을 붙였을 때 편집기가 무엇을 채워 주는지 화면을 띄워 보이며 자동 완성이 뜨는 자리를 하나씩 짚어 가며 차근차근 설명한다.",
        "묶지 말아야 할 경우를 들며 응집도가 없는 값을 억지로 모으면 생기는 문제를 반대 사례와 함께 화면을 바꿔 가며 설명하고 정리한다.",
        "마지막으로 이 패턴을 언제 되돌려야 하는지 이야기하며 객체가 커졌을 때 나타나는 신호를 하나씩 열거하고 발표를 맺으며 인사한다.",
      ],
    });

    expect(verdict.outcome === "ask-back" && verdict.askBack[0]?.reason).toContain(
      "관점의 두 배를 넘는다",
    );
  });

  it("뼈대가 상한까지 차도 관점이 그 절반이면 통과한다", () => {
    const verdict = judge({
      sourceOutline: Array.from(
        { length: 7 },
        (_unused, index) =>
          `${String(index)}번째 대목에서 원문은 호출부가 어떻게 읽히는지 예를 바꿔 가며 짚는다.`,
      ),
    });
  ...
  it("관점이 이미 얇다고 걸렸으면 균형은 다시 묻지 않는다", () => {
    ...
    expect(reasons).not.toContain("관점의 두 배를 넘는다");
  });
